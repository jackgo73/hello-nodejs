...
监督学习的目的就是学习一个模型，应用这一模型，对给定的输入预测相应的输出。这个模型的一般形式是决策函数：
$$
Y=f(x)
$$
或条件概率分布：
...
由数据直接学习决策函数$f(X)$或者条件概率分布$P(Y\vert X)$作为预测模型，即判别模型。判别方法是对给定的输入X，应该预测什么样的输出Y。典型的判别模型包括：
...
在监督学习中，生成方法和判别方法各有优缺点，适合不同条件下的学习问题。

## 优缺点

生成方法的特点：

- 可以还原出联合概率分布$P(Y\vert X)$
- 学习收敛速度快，即当样本容量增加的时候，学到的模型可以更快的收敛于真实模型
- 当存在隐变量时，仍可以用生成方法学习。

判别方法特点：

- 不能还原出联合概率分布，存在隐变量时不能使用判别方法学习。
- 直接面向预测，学习的准确率往往更高。
- 直接学习联合概率分布，所以可以对数据进行各种程度上的抽象、定义特征并使用特征，可以简化学习问题。

# 分类问题

- 分类是监督学习的一个核心问题，当输出变量Y区有限个离散值时，预测问题即为分类问题。
- 分类模型叫做“分类器”，预测的过程叫“分类”，可能的输出叫“类”。
- 评价分类器性能的指标是分类准确率，即0-1损失函数。
- 评价分类器常用指标是精确率和召回率。
  - 精确率$P=\frac {正预测为正}{正预测为正+负预测为正}$
  - 召回率$R=\frac {正预测为正}{正预测为正+正预测为负}$
  - F1值$\frac{1}{F_1}=\frac {1}{P}+\frac {1}{R}=\frac {2TP}{2TP+FP+FN}$
- k近邻法、感知机、朴素贝叶斯、决策树、决策列表、逻辑斯蒂回归模型、支持向量机、提升方法、贝叶斯网络、神经网络、winnow都可以用于分类。

# 标注问题

分类问题的一个推广，也是结构预测问题的简单形式。

- 输入一个观测序列，输出一个标记序列或状态序列。
- 目标在于学习一个模型，使他能够对观测序列给出标记序列作为预测。

举个例子：

![](/images/2017-10-19-Statistics01-Introduction-3.jpg)

# 回归问题

- 用于预测输入变量和输出变量之间的关系。
- 等价于函数拟合：选择一条函数曲线使其很好的拟合已知数据且很好的预测未知数据。
- 回归问题按照输入变量的个数，分为一元回归和多元回归。按照输入变量和输出变量之间的关系分为线性回归和非线性回归。
- 最常用的损失函数是平方损失函数，在此情况下回归问题可以用最小二乘法求解。
- 例子：
  - 市场趋势预测
  - 产品质量管理
  - 客户满意度调查
